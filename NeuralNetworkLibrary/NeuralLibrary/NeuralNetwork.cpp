...
#define Length(a) (sizeof(a) / sizeof(a[0]))	// Returns the length of the array
#define randDouble() ((double)(rand()%1000000000)/1000000000.0)	// Returns a random double from 0 to 1
...
#pragma region Constructors
	...
	public:
		Network(int layerSizes[], TransferFunction transferFunctions[])
		{
			//Validate the input data
			if (Length(transferFunctions) != Length(layerSizes) || transferFunctions[0] != TransferFunction::None)
				throw ("Cannot construct a network with these parameters");

			//Initialize network layers
			layerCount = Length(layerSizes) - 1;
...
#pragma region Methods
	...
	public:
		...
		double Train(double* input, double* &desired, double TrainingRate, double Momentum)
		{
			// Parameter Validation
			if (Length(input) != inputSize)
				throw ("Ivalid input parameter", "input");
			if (Length(desired) != layerSize[layerCount - 1])
