...
public:
	...
	void Draw(sf::RenderWindow& window, bool drawRays)
	{
		...
		b2Vec2 forceVec((float32)8 * cos(body->GetAngle() + (float32)GetWheelsAngle() - 90 / DEG), (float32)8 * sin(body->GetAngle() + (float32)GetWheelsAngle() - 90 / DEG));
		Vertex v[2];
	...
	#else
			int angle = round(NetworkOutput[0]*DEG) + WHEEL_ANGLE_BIAS;
			if (angle < 0)
				angle = 0;
